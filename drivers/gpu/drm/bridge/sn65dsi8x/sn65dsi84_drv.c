			   ...
			   struct sn65dsi84_data *data)
{
	...
	data->sn65dsi84_en_gpio = devm_gpiod_get_optional(dev, "EN",  GPIOD_ASIS);
	if (IS_ERR(data->sn65dsi84_en_gpio)) {
		printk(KERN_INFO "sn65dsi84_parse_dt: failed to get EN GPIO \n");
	}

	data->lvds_vdd_en_gpio = devm_gpiod_get_optional(dev, "lvds_vdd_en", GPIOD_ASIS);
	...
	data->pwr_source_gpio = devm_gpiod_get_optional(dev, "pwr_source", GPIOD_ASIS);
